...
def Type01_solve(tagged, words):
    ...
    for i in range(0, len(words)):
        ...
        # Common_Rule 04 - Get potential variables with attributes( Eg: කුඩාතැපැල්පත්,විශාලතැපැල්පත්)
    ...
    for item in splittedEdit:
        ...
        for i in range(0, len(item)):
            ...
            # Template Rule 05 - Replace multiplication
            if (item[i][1] == "MUL"):
                words_inuse[i] = "*"
                item[i] = ("*", "MUL")

            # Template Rule 06 - Replace Addition with "හා" and "සහ"
            if (item[i][1] == "ADD" or item[i][0] == "හා" or item[i][0] == "සහ"):
                words_inuse[i] = "+"
                temp = ("+", "ADD")
                item[i] = temp

            # Template Rule 07 - Replace Sub with "-"
            if (item[i][1] == "SUB"):
                words_inuse[i] = "-"
                temp = ("-", "SUB")
                item[i] = temp

            # Template Rule 07- Apply multiplication and generate Variable
            if ((item[i][1] == "VAR") and (item[i + 1][1] == "MUL") and (
                ...
                words_inuse[i + 1] = words_inuse[i + 2] = "NULL"
                item[i] = temp
            ...
            # Template Rule 08- Apply multiplication and generate Variable with "මෙන්"
            if ((item[i][0] == "*") and (item[i + 1][1] == "CD") and (
                    item[i + 2][1] == "NNN" and (item[i + 2][0] in variables) and (item[i + 2][0] != "ගණන"))):
                for k in range(0, len(variablesReplace)):
                    if (words_inuse[i + 2] == variablesReplace[k][0]):
                        result = str(variablesReplace[k][1])
                multiplied = str(item[i + 1][0]) + "" + result
                temp = (multiplied, "VAR")
                words_inuse[i] = multiplied
            ...
            # Template Rule 09- Apply multiplication
            if (((item[i][1] == "CD") or (item[i][1] == "VAR")) and (item[i + 1][1] == "EQ") and (
                    item[i + 2][0] == item[i + 3][0] == "NULL") and item[i + 4][0] == "."):
                exp = "= " + item[i][0]
                temp = (exp, "EXP")
                words_inuse[i] = exp
                item[i] = temp
                words_inuse[i + 1] = "NULL"
                item[i + 1] = ("NULL", "NULL")

            #  Template Rule 10 - Identifying Equivalance method.
            if ((item[i][1] == "VAR") and (
            ...
            #  Template Rule 11 - Identifying Equivalance method with "නම්" word
            if (item[i][0] == "නම්"):
                temp = ("=", "EQ")
                item[i] = temp
                words_inuse[i] = "="

            if (item[i][0] == "=" and item[i - 1][1] == "VAR" and item[i - 2][1] == "VAR"):
                temp1 = item[i - 1]
                temp2 = words_inuse[i - 1]
                item[i - 1] = item[i]
                words_inuse[i - 1] = words_inuse[i - 1]
                item[i] = temp1
                words_inuse[i] = temp2

            if (item[i][1] == "CD" and item[i - 1][1] == "ADD" and item[i - 2][1] == "VAR" and item[i - 3][1] == "ADD"):
                temp = ("=", "EQ")
                item[i - 1] = temp
                words_inuse[i - 1] = "="
            print(item)
    ...
    for item in splittedEdit:
        ...
        for i in range(0, len(item)):
            ...
            if (item[i][1] == "VAR" and item[i][0] not in varPuts):
        ...
        for i in range(0, len(finalPart)):
            ...
            if (finalPart[i] == "=" and (i - 1 < len(finalPart) and finalPart[i - 1] in varPuts) and (
                    i - 2 > 0 and finalPart[i - 2] in varPuts)):
                temp = finalPart[i - 1]
                finalPart[i - 1] = finalPart[i]
                finalPart[i] = temp
